...
package object data {
        ...
        case (DataMap(expected), DataMap(actual)) =>
          (expected.keys.toVector ++ actual.keys.toVector).distinct.toList.flatMap { key =>
            (expected.get(key), actual.get(key)) match {
              case (Some(expected), Some(actual)) => expected.diff(actual).map(_.prependKey(key))
              case (Some(expected), None)         => DiffEntry(key, expected, Data()).prependKey(key) :: Nil
              case (None, Some(actual))           => DiffEntry(key, Data(), actual).prependKey(key) :: Nil
              case (None, None)                   => Nil
            }
          }
  ...
  implicit class DiffOps(private val diff: Diff) extends AnyVal {
