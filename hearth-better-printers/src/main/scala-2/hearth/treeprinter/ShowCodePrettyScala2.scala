...
trait ShowCodePrettyScala2 {
  ...
  private object implementationDetails {
    ...
    final class HighlighedCodePrinter(
      ...
      // Kind-projector can generate:
      //   - Greek param names like "α$0$"
      //   - ASCII param names like "X_kp0$0$" when kp:genAsciiNames=true
      //   - Lambda alias name "Λ$" (default) or "L_kp" (ASCII mode, does not end with "$")
      // None of those should be treated as objects when printing.
      private val kindProjectorParamPattern = "(?:[α-ω]|X_kp\\d+)\\$\\d+\\$$".r
      private val kindProjectorLambdaName = "Λ$"
      ...
      override protected def printedName(name: Name, decoded: Boolean = true) = {
        val (result, dotType) = {
          ...
          else if (kindProjectorParamPattern.matches(x) || x == kindProjectorLambdaName) (x, false)
