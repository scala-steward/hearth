...
/** These macros are responsible for rewriting Expr.quote/Expr.splice into native quotes
  ...
  *     helper method's `Expr.quote` expands into a '''separate''' quasiquote referencing those local type params, the
  *     free types will fail to resolve. Keep `Expr.quote` calls that reference local type params inline in the splice
  *     body. Helper methods can do any non-quoting work (build strings, call `Expr(...)`, apply `Type.Ctor1`, etc.) —
  *     only the `Expr.quote` itself needs to stay inline.
  *   - '''`outerEnclosingMethodImplicitTypes` is only in `ImplicitTypeReferenceReplacer`:''' Deliberately excluded from
  *     `TypeWithUnderlyingInjected` (used by `typeOfImpl` for standalone `Type.of` calls). Adding it there caused
  *     regressions in library code using `Type.of` with existential types (e.g. `Type.of[java.lang.Enum[?]]`), because
  *     walking the owner chain from unrelated call sites found spurious candidates.
  */
final class CrossQuotesMacros(val c: blackbox.Context) extends ShowCodePrettyScala2 with CrossQuotesMacrosCtorMethods {
  ...
  /* When `c.weakTypeTag[Tpe]` is used, the compiler ignores WeakTypeTags for types that were imported
   * (e.g. `tpe.Underlying`), defined as implicit val/def/given, or obtained via HKT candidates. The result
   * is that `tpe.Underlying` inside `Tpe` resolves to the local variable's WeakTypeTag instead of the actual
   * underlying type.
   *
   * To work around this, we consider imported underlying types, enclosing method implicit types, and HKT
   * candidates, then rewrite the tree into:
   *
   * {{{
  ...
  private class SpliceReplacer(ctx: TermName) extends Transformer {
    ...
    object Splices {
      ...
      /** Generates a workaround method for splices that reference local type params.
        ...
        * the need to rewrite tree nodes in the showCodePretty output (which is fragile because showCodePretty uses
        * TypeTree.original fields). Note that `stripFreeTypes` is still applied afterward to sanitize any type trees
        * generated by Hearth utilities (CaseClass, Method, LambdaBuilder, etc.) that may contain free type symbols.
        */
      private def storeWithWorkaround(
        ...
        // Wrap the result with CrossQuotes.stripFreeTypes to replace TypeTree nodes containing free type symbols
        // with untyped type trees. Without this, trees generated by Hearth utilities (CaseClass, Method,
        // LambdaBuilder, etc.) would leak free type symbols into the final macro expansion, causing
        // "Macro expansion contains free type variable" errors.
        val code =
          s"""$${ {
             |  def $workaroundStr[$typeParamDecls] = {
             |    $printedExpr
             |  }
             |  CrossQuotes.stripFreeTypes($ctx)($workaroundStr[$typeArgs]($freeTypeArgs).asInstanceOf[$ctx.Expr[Any]])
             |} }""".stripMargin
      ...
      def storeVarArgs(expr: Tree, tpe: Tree): Tree = {
        ...
        if (localRefs.isEmpty) {
          ...
          // Workaround case: VarArgs splice references local type params
          storeVarArgsWithWorkaround(expr, tpe, localRefs)
        }
      }

      /** Generates a workaround method for VarArgs splices that reference local type params.
        *
        * Like `storeWithWorkaround` but adapted for the VarArgs splice pattern: instead of producing an `Expr`, the
        * result is unwrapped via `.tree.children.tail` to yield the spliced sequence elements.
        */
      private def storeVarArgsWithWorkaround(
          expr: Tree,
          tpe: Tree,
          localRefs: Set[(Symbol, String)]
      ): Tree = {
        ...
        val code =
          s"""..$${ {
             ...
             |    $printedExpr
             |  }
             |  CrossQuotes.stripFreeTypes($ctx)($workaroundStr[$typeArgs]($freeTypeArgs).asInstanceOf[$ctx.Expr[Seq[Any]]]).tree.children.tail
             |} }""".stripMargin

        log(
          s"""Cross-quotes ${paintExclDot(Console.BLUE)("Expr.splice")} VarArgs workaround for local type params:
             |Type params: ${sortedRefs.map(_._2).mkString(", ")}
             |Code: ${indent(code)}""".stripMargin
        )
