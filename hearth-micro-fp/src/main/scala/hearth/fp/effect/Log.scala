...
object Log {
  ...
  private[effect] def render(rootScopeName: String, logs: Logs)(filter: Level => Boolean): String = {
    ...
    final case class Item(log: Log, indentHead: String, indentTail: String) {
      ...
      def step: Vector[Item] = log match {
    ...
    // Allows rendering arbitrarily large nested logs without stack overflow
    @scala.annotation.tailrec
    def loop(queue: Vector[Item]): String = queue match {
      case currentItem +: remaining => loop(currentItem.step ++ remaining)
      case _                        => sb.toString()
    }

    loop(Vector(Item(Scope(rootScopeName, filterLevels(logs, filter)), "", "")))
  }

  private def filterLevels(logs: Logs, filter: Level => Boolean): Logs = logs.view
    .map {
      case Scope(name, entries) => Scope(name, filterLevels(entries, filter))
      case entry                => entry
    }
    .collect {
      case entry: Entry if filter(entry.level)    => entry
      case scope: Scope if scope.entries.nonEmpty => scope
    }
    .toVector
}
