...
/** Macro extension providing support for Either[String, Value] constructors.
  *
  * Supports all types that have a single constructor argument.
  *
  * @since 0.3.0
  */
final class CtorLikeProviderForEitherStringOrValue extends StandardMacroExtension { loader =>
  ...
  override def extend(ctx: MacroCommons & StdExtensions): Unit = {
    ...
    CtorLikes.registerProvider(new CtorLikes.Provider {
      ...
      override def parse[A](tpe: Type[A]): ProviderResult[CtorLikes[A]] = {
        ...
        import EitherStringOrValue.Result
