# Guidelines for code style
 ...
 4. CI checks whether 2. and 3. are respected, but you can use `sbt --client "quick-test ; scalafmtAll"` to quickly check that code compiles and format it.
 ...
 8. I prefer to use:
    ...
    because it automatically imports everything from `a` and `b`, which reminds us that the package visibility is hierarchical in Scala.
 9. If a solution works great with Scala 3, but does not work at all with Scala 2.13 - it goes against the goals of this library.
    Even when we would implement features that only work on Scala 3 (e.g. opaque types, named tuples) - it has to be done in a way
