...
[pipez](https://github.com/MateuszKubuszok/pipez) was an earlier research project by Hearth's author exploring how far cross-compilation could be pushed. It experimented with abstracting over the differences between Scala 2 and Scala 3 macro systems, testing which patterns could be shared and where platform-specific code was unavoidable. The lessons from pipez — about what abstractions were practical and where they broke down — fed directly into Hearth's design.

## The Synthesis

Looking at the arc: scala-commons showed that macro utilities could be reusable. Shapeless and Circe established the vocabulary and patterns for type class derivation. Magnolia improved the developer experience of writing derivations. jsoniter-scala proved that macro-internal recursion was practical. Chimney and chimney-macro-commons demonstrated that cross-compilation could work for real-world macros. And pipez explored the design space for cross-platform macro abstractions.
