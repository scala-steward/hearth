...
This approach was pioneered by [jsoniter-scala](https://github.com/plokhotnyuk/jsoniter-scala) for codec generation, and refined by [Chimney](https://chimney.readthedocs.io/) for type transformations. Hearth's utilities (`CaseClass.parse`, `Enum.parse`, `SingletonValue.parse`, `parMatchOn`, etc.) are designed to make implementing this pattern straightforward.

For the full rationale, see [the blog post](https://kubuszok.com/2025/sanely-automatic-derivation/). For the ecosystem story behind this approach, see [Prior Art & Influences](prior-art.md). For a step-by-step implementation guide, see the [Derivation Checklist](derivation-checklist.md).
...
For broader guidance on publishing and maintaining Scala libraries, see the [Library Author Guide](https://docs.scala-lang.org/overviews/contributors/index.html) and [Resources & Further Reading](resources.md).
