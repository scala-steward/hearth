...
When using [`MacroCommons`](basic-utilities.md#library-conventions) you don't need to use `mio.unsafe.runSync`.
...
There are also utilities to handle some utilities with MIO:
 ...
 - instead of passing around a lot of copies of [an immutable `ValDefsCache`](basic-utilities.md#valdefscache), we can create the cache
   as `MLocal` via `ValDefsCache.mlocal`, and then use extension methods on it to "mutate" the `MLocal` value in an MIO program
